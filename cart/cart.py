...
class Cart:
    ...
    def __iter__(self):
        ...
        for item in cart_dict.values():
            item['total'] = item['product'].new_price * item['quantity']
            yield item
    ...
    def get_total_price(self):
        ...
        for product in products:
            cart_dict[str(product.id)]['product'] = product
        price = sum(item['product'].new_price * item['quantity'] for item in cart_dict.values())
        return price
    ...
    # def item_count(self):
    #     sum_quantity = sum(item['quantity'] for item in self.cart.values())
    #     return sum_quantity
